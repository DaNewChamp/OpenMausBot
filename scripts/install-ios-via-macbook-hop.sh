...
run_on_macbook() {
  ...
  ssh -o BatchMode=yes -o ConnectTimeout=15 "${MACBOOK_USER}@${host}" \
    "$REMOTE_ENV; cd $REPO && git fetch origin $BRANCH && git checkout $BRANCH && git pull --ff-only origin $BRANCH && DEVICE_UDID=$DEVICE_UDID BRANCH=$BRANCH ./scripts/install-ios-device.sh"
}
