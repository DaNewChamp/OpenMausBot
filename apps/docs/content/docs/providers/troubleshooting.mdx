...
## Grok Reconstructed stays unavailable

The reconstructed engine is a local desktop adapter. Confirm **Grok Bot 0.18
Reconstructed** is running on the same Mac as OpenMausBot. Official Grok Bot is
not used. iOS should remain paired to OpenMausBot; do not enter a reconstructed
loopback port on the phone.
