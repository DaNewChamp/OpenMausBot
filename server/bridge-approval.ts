...
export type BridgeApprovalTool = "run_on_bridge" | "run_on_ssh_target" | "observe_bridge_screen";
...
function pushApprovalCard(
  ...
  const title =
    req.tool === "run_on_ssh_target"
      ...
      : req.tool === "observe_bridge_screen"
  ...
  for (const bot of bus.store.bots) {
    ...
    for (const threadId of threadIds) {
      for (const message of bus.store.messagesFor(threadId)) {
        ...
        if (card.tool !== "run_on_bridge" && card.tool !== "run_on_ssh_target" && card.tool !== "observe_bridge_screen") continue;
