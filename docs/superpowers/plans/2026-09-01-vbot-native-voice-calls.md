...
- Companion `/api/tts/prepare` availability is sidecar `ttsPrepareVersion` on `GET /api/companion/endpoints`. Hub `features.iosVoiceCalls` does not imply the sidecar allowlists or implements prepare. Missing, wrong-type, or non-int versions decode to `nil` without throwing; unknown numeric versions remain numeric and fail closed at the exact-version gate. A failed endpoint refresh must clear in-memory `ttsPrepareVersion` so a stale `1` cannot keep calls enabled.
