...
export function CompanionSection({ profileEmail = "" }: { profileEmail?: string }) {
  ...
  if (!companionBridge()) {
    return (
      <Card
        title="Use V Bot from your phone"
        subtitle="Open Settings in the V Bot desktop app to set up a phone."
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card
        title="Paired phones"
        subtitle={pairedCount ? "Manage the phones that can use this V Bot." : "No phones are paired yet."}
      >
        {pairedCount > 0 && (
                ...
                <div className="mt-3 flex items-center justify-between gap-3 border-t border-hairline/30 pt-3">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void c.act((companion) =>
                        companion.localVm
                          ? companion.localVm(device.id, !device.localVmAccess)
                          : companion.state(),
                      )
                    }
                    className={cnSwitch(device.localVmAccess ?? false)}
                  >
                    <span className={cnKnob(device.localVmAccess ?? false)} />
                  </button>
                </div>
              </li>
            ))}
