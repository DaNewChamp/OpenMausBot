...
export function BrowserPanel({
    ...
    async (action: "take" | "release"): Promise<boolean> => {
      ...
      try {
        ...
        const setLocalControl = async (held: boolean): Promise<boolean> => {
          try {
            if (!bridge?.setHumanControl) throw new Error("Update Vi Bot before using browser takeover.");
  ...
  const moveHistory = useCallback(async (direction: "back" | "forward") => {
    ...
    try {
      ...
      else throw new Error("Update Vi Bot before using Forward.");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause));
    } finally {
      setBusy(false);
  ...
  if (!bridge) {
    return (
      <div className="rounded-xl bg-card p-4 text-[13px] text-ink-secondary">
        The built-in browser needs the Vi Bot desktop app.
      </div>
    );
  }
