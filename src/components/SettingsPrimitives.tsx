...
export function Card({
  ...
  return (
    <div className="rounded-xl bg-card p-4">
      ...
      {subtitle && <div className={title ? "mt-1 text-[13px] leading-relaxed text-ink-secondary" : "text-[13px] leading-relaxed text-ink-secondary"}>{subtitle}</div>}
      {children && <div className={title || subtitle ? "mt-4 min-w-0" : "min-w-0"}>{children}</div>}
    </div>
  );
}
